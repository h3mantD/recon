...
altdns -i all.txt -o data_output -w ~/WorkSpace/tools/recon/patterns.txt -r -s results_output.txt
...
~/WorkSpace/tools/massdns/bin/massdns -r ~/WorkSpace/tools/massdns/lists/resolvers.txt -q -t A -o S -w massdns.raw all.txt
...
mkdir nuclei_op
nuclei -l alive.txt -t "~/WorkSpace/tools/nuclei-templates/cves/*.yaml" -c 60 -o nuclei_op/cves.txt
nuclei -l alive.txt -t "~/WorkSpace/tools/nuclei-templates/files/*.yaml" -c 60 -o nuclei_op/files.txt
nuclei -l alive.txt -t "~/WorkSpace/tools/nuclei-templates/panels/*.yaml" -c 60 -o nuclei_op/panels.txt
nuclei -l alive.txt -t "~/WorkSpace/tools/nuclei-templates/security-misconfiguration/*.yaml" -c 60 -o nuclei_op/security-misconfiguration.txt
nuclei -l alive.txt -t "~/WorkSpace/tools/nuclei-templates/technologies/*.yaml" -c 60 -o nuclei_op/technologies.txt
nuclei -l alive.txt -t "~/WorkSpace/tools/nuclei-templates/tokens/*.yaml" -c 60 -o nuclei_op/tokens.txt
nuclei -l alive.txt -t "~/WorkSpace/tools/nuclei-templates/vulnerabilities/*.yaml" -c 60 -o nuclei_op/vulnerabilities.txt

echo "Now looking for CORS misconfiguration"
python3 ~/WorkSpace/tools/Corsy/corsy.py -i alive.txt -t 40 | tee -a corsy_op.txt
...
python3 ~/WorkSpace/tools/smuggler.py -u alive.txt | tee -a smuggler_op.txt
...
do
        ...
        do
                ruby ~/WorkSpace/tools/relative-url-extractor/extract.rb scriptsresponse/$domain/$file >> endpoints/$domain/$file
        done
done
...
for i in $(cat alive.txt);do ffuf -u $i/FUZZ -w ~/WorkSpace/tools/dirsearch/db/dicc.txt -mc 200 -t 60 ;done| tee -a ffuf_op.txt
